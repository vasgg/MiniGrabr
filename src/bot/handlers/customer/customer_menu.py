...
async def customer_menu_handler(
    ...
    await state.set_state(States.sender_mode)
...
async def customer_my_orders_handler(
    ...
    await call.message.answer_photo(
        photo=types.FSInputFile(path="core/internal/pictures/orders.jpeg"),
        caption=text,
...
async def customer_my_projects_handler(
    call: types.CallbackQuery, user: User, session
) -> None:
    active_orders = await get_active_orders(
        mode=UserType.CUSTOMER, customer_id=user.id, session=session
    )
    ...
    text = answer["сustomer_projects_reply"] + await get_projects_list_string(
        mode=UserType.CUSTOMER, applications=projects, orders=active_orders
    )
    keyboard = get_applications_keyboard(
        mode="customer_projects", applications=projects, orders=active_orders
    )
    await call.message.answer_photo(
...
async def customer_applications_handler(
    ...
    orders = await get_orders(
        mode="my", user_id=user.id, status=OrderStatus.PUBLISHED, session=session
    )
    ...
    text = answer["customer_applications_reply"] + get_applications_list_string(
        mode=UserType.CUSTOMER, applications=applications, orders=orders
    )
...
async def customer_send_message_handler(
    call: types.CallbackQuery, state: FSMContext
) -> None:
    application_id = int(call.data.split(":")[1])
    msg = await call.message.answer(text=answer["customer_send_message_reply"])
    await state.update_data(
        customer_message_application_id=application_id,
        customer_send_message_id=msg.message_id,
    )
    await state.set_state(States.sender_send_message)
    await call.answer()


@router.message(States.sender_send_message)
async def customer_send_message(
    message: types.Message, state: FSMContext, session
) -> None:
    ...
    text = answer["bot_send_message_reply"].format(
        "Заказчик:", customer.fullname, application_id, customer_text
    )
    await send_message(
        message,
        receiver_id=freelancer.telegram_id,
        text=text,
        reply_markup=get_answer_keyboard(
            mode="customer", application_id=application_id
        ),
    )
    await message.delete()
    await message.bot.delete_message(
        chat_id=message.from_user.id, message_id=data.get("customer_send_message_id")
    )
    await message.answer(
        text=answer["bot_message_sent_reply"].format(
            customer_text, application_id, freelancer.fullname
        ),
...
async def customer_mark_as_done_handler(
    call: types.CallbackQuery, user: User, session
) -> None:
    application_id = int(call.data.split(":")[1])
    application = await get_application(
    ...
    await change_order_status(
        entity=application.order,
        entity_id=application.order_id,
        status=OrderStatus.DONE,
        session=session,
    )
    await change_user_balance(
        mode="add",
        telegram_id=freelancer.telegram_id,
        current_balance=freelancer.balance,
        amount=application.fee,
        session=session,
    )
    await call.message.answer(
        text=answer["customer_done_order_reply"].format(application_id),
        reply_markup=close_button,
    )
    await send_message(
        call.message,
        freelancer.telegram_id,
        text=answer["done_order_message_from_customer"].format(
            user.fullname, application_id, application.fee
        ),
        reply_markup=close_button,
    )
    await call.answer()
