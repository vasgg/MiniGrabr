...
from bot.controllers.order_controllers import check_order_before_publish, missing_fields, order_to_fields, \
    publish_order_to_db
from bot.internal.dict import answer
from bot.keyboards.customer.new_order_keyboard import get_publish_order_buttons
    ...
    match current_state:
        case "States:change_order_name":
            ...
            await save_params_to_draft(
                order_id=draft.id, mode="name", value=new_name, session=session
            )
        ...
        case "States:change_order_budget":
            ...
            await save_params_to_draft(
                order_id=draft.id, mode="budget", value=new_budget, session=session
            )
            await message.answer(
        ...
        case "States:change_order_link":
            ...
            if validate_url(new_link):
                await save_params_to_draft(
                    order_id=draft.id, mode="link", value=new_link, session=session
                )
                ...
                await message.answer(
                    answer["incorrect_url_reply"].format(message.text),
...
async def publish_order_handler(
    ...
    draft = await get_customer_draft(user_id=user.id, session=session)
    order_fields = order_to_fields(draft)
    not_filled = missing_fields(order_fields)
    if len(not_filled) == 0:
        ...
        msg = await call.message.edit_text(
            text=text, reply_markup=get_publish_order_buttons(draft.id)
        )
...
async def apply_worker_handler(call: types.CallbackQuery, user: User, session) -> None:
    ...
    application = await get_active_application(
    ...
    if not check_balance_before_apply_worker(
        application_fee=application.fee, user_balance=user.balance
    ):
        await call.message.answer(
            text=answer["insufficient_funds_reply"], reply_markup=close_button
        )
        return
    ...
    await send_message(
        ...
        text=text_to_executor,
        reply_markup=get_executior_keyboard(application_id, mode=UserType.FREELANCER),
    )
    await call.message.edit_text(
        text=answer["apply_worker_reply_to_customer"],
