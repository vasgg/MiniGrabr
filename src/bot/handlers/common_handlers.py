...
from bot.controllers.user_controllers import rename_user
from bot.internal.dict import answer
from bot.internal.context import States
from bot.keyboards.common_keyboards import close_button, get_back_to_menu_and_pay_buttons, role_selector_kb
...
async def customer_balance_handler(
    call: types.CallbackQuery, state: FSMContext
) -> None:
    await call.message.answer_photo(
        photo=types.FSInputFile(path="core/internal/pictures/balance.jpeg"),
...
async def add_funds_to_balance_handler(
    ...
    try:
        ...
        if amount < 93 or amount > 924472:
            raise ValueError
        await message.answer(
            text=answer["check_balance_reply"].format(
                "Вы хотите пополнить баланс на ", amount
            ),
        ...
        await message.answer(
            text=answer["incorrect_balance_reply"].format(message.text)
        )
