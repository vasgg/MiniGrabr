...
async def fl_find_order_handler(
    ...
    orders = await get_orders(
        user_id=user.id, mode="others", status=OrderStatus.PUBLISHED, session=session
    )
    ...
    text = answer["fl_find_orders_reply"] + get_orders_list_string(
        filtered_orders, mode="traveler"
    )
    orders_list = await call.message.answer(
        text=text, reply_markup=get_orders_keyboard(filtered_orders, mode="traveler")
    )
...
async def fl_my_projects_handler(
    ...
    active_orders = await get_active_orders(
        mode=UserType.FREELANCER, worker_id=user.id, session=session
    )
    order_ids = [order.id for order in active_orders]
    projects = []
    for order_id in order_ids:
        application = await get_application(
            mode="by_order_id", order_id=order_id, session=session
        )
        projects.append(application)
    text = answer["fl_projects_reply"] + await get_projects_list_string(
        mode=UserType.FREELANCER, applications=projects, orders=active_orders
    )
    keyboard = get_applications_keyboard(
        mode="freelancer_projects", applications=projects, orders=active_orders
    )
    await call.message.answer_photo(
        photo=types.FSInputFile(path="core/internal/pictures/projects.jpeg"),
        caption=text,
...
async def fl_mark_as_done_handler(
    call: types.CallbackQuery, user: User, session
) -> None:
    application_id = int(call.data.split(":")[1])
    application = await get_application(
        mode="by_app_id", application_id=application_id, session=session
    )
    customer = await get_user(application.customer_id, session=session)
    await call.message.answer(
        text=answer["fl_done_order_reply"].format(application_id),
        reply_markup=close_button,
    )
    await send_message(
        call.message,
        customer.telegram_id,
        text=answer["done_order_message_from_freelancer"].format(
            user.fullname, application_id
        ),
...
async def fl_applications_handler(
    ...
    text = answer["fl_applications_reply"] + get_applications_list_string(
        mode=UserType.FREELANCER, applications=applications, orders=orders
    )
    if call.data == "fl_applications":
        photo = types.FSInputFile(path="core/internal/pictures/applications.jpeg")
        keyboard = get_applications_keyboard(
            orders=orders, applications=applications, mode="traveler"
        )
    else:
        photo = types.FSInputFile(path="core/internal/pictures/messages.jpeg")
        keyboard = get_applications_keyboard(
...
async def fl_send_message_handler(call: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await state.update_data(
        fl_message_application_id=application_id, fl_send_message_id=msg.message_id
    )
    await state.set_state(States.traveler_send_message)
    await call.answer()


@router.message(States.traveler_send_message)
async def fl_send_message(message: types.Message, state: FSMContext, session) -> None:
    ...
    text = answer["bot_send_message_reply"].format(
        "Фрилансер:", freelancer.fullname, application_id, fl_text
    )
    await send_message(
        message,
        receiver_id=customer.telegram_id,
        text=text,
        reply_markup=get_answer_keyboard(
            mode="traveler", application_id=application_id
        ),
    )
    await message.delete()
    await message.bot.delete_message(
        chat_id=message.from_user.id, message_id=data.get("fl_send_message_id")
    )
    await message.answer(
        text=answer["bot_message_sent_reply"].format(
            fl_text, application_id, customer.fullname
        ),
        reply_markup=close_button,
    )
...
async def my_account_handler(call: types.CallbackQuery, user: User, session) -> None:
    ...
    deals = answer["deals_as_freelancer"].format(
        await get_deals_counter(user_id=user.id, mode="traveler", session=session)
    )
    await call.message.answer_photo(
        photo=types.FSInputFile(path="core/internal/pictures/account.jpeg"),
